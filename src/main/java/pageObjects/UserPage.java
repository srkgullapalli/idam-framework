...
public class UserPage {
    ...
    @FindBy(xpath = "//table[@id='mytable']//td[8]")
    private WebElement table_TicketStatus;

    @FindBy(xpath = " //table[@id='mytable']//td[11]")
    private WebElement table_MASTicketStatus;

    @FindBy(xpath = "//table[@id='mytable']//td[8]")
    private WebElement table_PushStatus;

    @FindBy(xpath = "//table[@id='mytable']//td[9]")
    private WebElement table_PushedAt;

    @FindBy(xpath = "//table[@id='mytable']//td[10]")
    private WebElement table_PushResponse;

    @FindBy(xpath = "//table[@id='mytable']//td[13]")
    private WebElement table_TicketID;
    ...
    public void verifySearchStatus(TestContext testContext, DataTable table) {
        final Map<String, String> map = table.asMap(String.class, String.class);

        for (String key : map.keySet()) {
            switch (key) {
                case "approvalStatus":
                    scrollToElementActions(driver, table_userApprovalStatus);
                    Assert.assertEquals(map.get("approvalStatus"), table_userApprovalStatus.getText());
                    break;
                case "IDAMID":
                    scrollToElementActions(driver, table_IDAMID);
                    Assert.assertNotNull("IDAM ID is not generated for user", table_IDAMID);
                    break;
                case "userStatus":
                    scrollToElementActions(driver, table_userStatus);
                    Assert.assertEquals(map.get("userStatus"), table_userStatus.getText());
                    break;
                case "Ticket Status":
                    scrollToElementActions(driver, table_TicketStatus);
                    Assert.assertEquals(map.get("Ticket Status"), table_TicketStatus.getText());
                    break;
                case "Ticket ID":
                    scrollToElementActions(driver, table_TicketID);
                    Assert.assertNotNull("Ticket ID is not displayed", table_TicketID);
                    break;
                case "Mass Ticket Status":
                    scrollToElementActions(driver, table_MASTicketStatus);
                    Assert.assertEquals(map.get("Mass Ticket Status"), table_MASTicketStatus.getText());
                    break;
                case "Push Status":
                    scrollToElementActions(driver, table_TicketStatus);
                    Assert.assertEquals(map.get("Push Status"), table_PushStatus.getText());
                    break;
                case "Pushed At":
                    scrollToElementActions(driver, table_TicketID);
                    Assert.assertNotNull("Ticket ID is not displayed", table_PushedAt);
                    break;
                case "Push Response":
                    scrollToElementActions(driver, table_TicketStatus);
                    Assert.assertEquals(map.get("Push Response"), table_PushResponse.getText());
                    break;
            }
        }
    }
    ...
    public void searchUser(DataTable inputTable, TestContext testContext) {
        final Map<String, String> map = inputTable.asMap(String.class, String.class);
